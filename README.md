...
ログインすると **作品・人・項目** のお気に入りが同一アカウントで別ブラウザと共有されます。認証は **パスワードが主** です（パスキーのみの既存アカウントはログイン失敗後にフォールバック表示）。ヘッダー右上の「ログイン」から `/login` へ進めます（未ログインでも検索は利用可）。パスワード再設定メールを送る場合は `RESEND_API_KEY` と `AUTH_EMAIL_FROM` を設定してください。HTTPS 上ではセッション Cookie が `Secure` になります（`NODE_ENV=production`）。`NEXT_PUBLIC_SITE_URL` を本番ドメインに設定してください（RP ID 用）。
