## Fanza Search Navigator

Fanza APIを使って作品検索・閲覧・公式購入導線をまとめた **R18 Webアプリ** です。
検索一覧から画像/サンプル動画/アフィリエイト（広告）リンクへアクセスできます。

**公式ストア（App Store / Google Play / Microsoft Store）には出しません。** 成人向けコンテンツは各ストアの現行ガイドラインで拒否されるため、公開は Web（任意でホーム画面追加）に限定します。詳細は `docs/STORE_AND_LEGAL.md`。

## セットアップ

1. 依存パッケージのインストール

```bash
npm install
```

2. 環境変数を設定

`.env.example` をコピーして `.env.local` を作成し、値を入力してください。

```bash
cp .env.example .env.local
```

必須:
- `DMM_API_ID`
- `DMM_AFFILIATE_ID`

任意:
- `MULTI_SEARCH_MODE`（`auto` / `unified` / `source_tabs`）
- `PROVIDER_TIMEOUT_MS` / `MULTI_FALLBACK_SLOW_MS`
- `NEXT_PUBLIC_SITE_URL`（本番の正規URL・OG等）
- `NEXT_PUBLIC_FEEDBACK_URL`（フッターの要望リンク。未設定時は GitHub Issues）

**GitHub 公開時:** `.env.local` は秘密を含むためリポジトリに含めません（`.gitignore` で除外）。`git add -f .env.local` のような強制追加はしないでください。テンプレは `.env.example` のみコミットします。

3. 開発サーバー起動

```bash
npm run dev
```

ブラウザで `http://localhost:3000` を開きます。

## 主な機能

- **FANZA（DMM ItemList）のみ**を検索元とするキーワード検索、人気・新着の閲覧、並び替え、ページング
- 一覧・詳細でメーカー／収録時間／サンプル有無など比較しやすい情報を表示
- 詳細ページの関連作品（出演またはジャンル）
- 購入リンクは **広告** 表示つきアフィリエイト（`rel=sponsored`）
- 18+ 年齢確認（自己申告＋規約同意）。検索クローラにはインタースティシャルを出さない
- **最近の検索**、**/** キーで検索欄フォーカス、**一覧の共有**、**フィードバック導線**
- 任意ログイン（パスワード中心）でお気に入りを端末間同期。ゲストでも検索可
- クリック計測用のAPIエンドポイントを提供

## FANZA専用について

検索結果の取得元は **DMM アフィリエイト Webサービス（FANZAフロア等）のみ**です。他サービス公式APIのプロバイダ登録は本リポジトリからは外しています（別アプリやプロキシで連携する場合は、DMM規約と各社規約に従ってください）。

参照: [DMM Affiliate Web Service](https://affiliate.dmm.com/api/)

### 参考：他社の公開API調査メモ（本アプリには未接続）

横断メタ検索を別プロジェクトで行う場合の参考まで。

| 名前 | 公式情報 |
| --- | --- |
| **DUGA** | [APEX ウェブサービス（Web API）](https://duga.jp/aff/api.html) |
| **SOKMIL** | [ソクミルアフィリエイト](https://sokmil-ad.com/)（登録後の案内を優先） |
| **SOD** | [コーポレート](https://corporate.sod.co.jp/) 等 — ItemList 型の公開RESTは確認しづらく、多くはASP経由の紹介案件 |

## 公開時の注意

- API利用規約およびアフィリエイト規約を確認してください
- 年齢制限や免責を明示するページを公開してください
- 本番では計測基盤（GA4/Plausible等）への接続を推奨します
- DMM/FANZA API・画像の**クレジット／利用条件**に従ってください（`ApiCredit` 等）。
  - 商用利用可否、画像・サムネイル利用、キャッシュ可否と保存期間（DMM側ポリシーを確認）

公開手順の要約は `docs/RELEASE_FLOW.md`（`master` マージ → Vercel production。公式ストアには出さない）。

## 本番デプロイ（準備済みの選択肢）

### 環境変数（本番）

`.env.example` と同じキーをホスティング側の「環境変数」に設定してください。`DMM_API_ID` と `DMM_AFFILIATE_ID` は必須です。

`NEXT_PUBLIC_ENABLE_ACCOUNT_SYNC` は既定で `0`（無効）です。**無料で軽く運用する推奨設定はこのまま**です。この場合、お気に入りは端末ローカル保存のみで、同期 API は使いません。

任意アカウント・お気に入り同期を使う場合のみ `NEXT_PUBLIC_ENABLE_ACCOUNT_SYNC=1` にし、データ保存先として **サーバー上の JSON ファイル**（既定ではプロジェクト直下の `.data/users.json`）を永続化してください。パスを変えたい場合は `FANZA_APP_DATA_DIR` を設定してください（相対パスはプロジェクトルートからの相対、推奨は絶対パス）。

### Docker（VPS など）

`next.config.ts` で `output: "standalone"` を有効にしているため、コンテナ向けの本番ビルドが可能です。

```bash
docker build -t fanza-web .
docker run -d --name fanza-web -p 3000:3000 \
  --env-file .env.production \
  -v fanza-app-data:/data \
  fanza-web
```

イメージ内では `FANZA_APP_DATA_DIR=/data` を既定にしており、`-v` でマウントしたボリュームにお気に入り同期データが残ります。同期を使う場合は `--env NEXT_PUBLIC_ENABLE_ACCOUNT_SYNC=1` も設定してください。

### Vercel などのサーバーレス

検索・詳細はサーバー側で DMM API を呼び出します。**Edge ではなく Node のサーバーレス**で動かしてください。

アカウント同期（`NEXT_PUBLIC_ENABLE_ACCOUNT_SYNC=1`）を使う場合は、**Vercel Blob（private）** にユーザー／お気に入り JSON を保存します（`BLOB_READ_WRITE_TOKEN`）。Docker/VPS では従来どおり `FANZA_APP_DATA_DIR` のファイル保存も使えます。

ログインすると **作品・人・項目** のお気に入りが同一アカウントで別ブラウザと共有されます。認証は **パスワードが主** です。FANZA公式の会員ログイン連携は第三者サイト向けに公開されていないため、当サイトアカウントはFANZA会員とは別です（同じメールで作ると管理しやすいです）。一度ログインすると、既定でこの端末では約13か月自動ログインします。パスキーのみの既存口座は、パスキーが無い端末でもログイン画面でパスワードを足して入れます（再設定メール `RESEND_API_KEY` / `AUTH_EMAIL_FROM` がある場合はメール経由）。ヘッダー右上の「ログイン」から `/login` へ進めます（未ログインでも検索は利用可）。HTTPS 上ではセッション Cookie が `Secure` になります（`NODE_ENV=production`）。`NEXT_PUBLIC_SITE_URL` を本番ドメインに設定してください（RP ID 用）。

### GitHub Actions

`master` / `main` への push とそれら向けの PR で `npm run lint` と `npm run build`（ダミーの API 環境変数付き）が走ります。
