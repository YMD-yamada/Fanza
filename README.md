...
ログインすると **作品・人・項目** のお気に入りが同一アカウントで別ブラウザと共有されます。認証は **パスワードが主** です。パスキーのみの既存口座は、パスキーが無い端末でもログイン画面でパスワードを足して入れます（再設定メール `RESEND_API_KEY` / `AUTH_EMAIL_FROM` がある場合はメール経由）。ヘッダー右上の「ログイン」から `/login` へ進めます（未ログインでも検索は利用可）。HTTPS 上ではセッション Cookie が `Secure` になります（`NODE_ENV=production`）。`NEXT_PUBLIC_SITE_URL` を本番ドメインに設定してください（RP ID 用）。
